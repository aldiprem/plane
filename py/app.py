...
try:
    ...
    print("⚠️ pytoniq tidak terinstall. Install dengan: pip install pytoniq")

# ==================== ENDPOINT BARU UNTUK MEMBUAT PAYLOAD ====================
...
# ==================== SEMUA ENDPOINT DILETAKKAN DI SINI ====================
...
def check_payment_status():
    """Cek status pembayaran menggunakan reference atau bodyBase64Hash"""
    data = request.json
    reference = data.get('reference')
    body_hash = data.get('bodyBase64Hash')
    
    if not reference and not body_hash:
        return jsonify({'success': False, 'error': 'Reference or body hash required'}), 400
    
    with db.get_connection() as conn:
        if reference:
            cursor = conn.execute('''
                SELECT * FROM payment_tracking WHERE reference = ?
            ''', (reference,))
        else:
            cursor = conn.execute('''
                SELECT * FROM payment_tracking WHERE body_base64_hash = ?
            ''', (body_hash,))
        
        tracking = cursor.fetchone()
        
        if tracking:
            # Cek juga di transactions
            tx_cursor = conn.execute('''
                SELECT t.* FROM transactions t
                JOIN payment_tracking p ON p.reference = ?
                WHERE t.memo LIKE ? OR t.memo LIKE ?
                ORDER BY t.created_at DESC LIMIT 1
            ''', (reference, f'%{reference}%', f'%{body_hash}%'))
            
            transaction = tx_cursor.fetchone()
...
if __name__ == '__main__':
    ...
    print(f"📦 TON Library available: {TON_LIB_AVAILABLE}")
    if not TON_LIB_AVAILABLE:
        print("⚠️ Install pytoniq untuk payload yang lebih baik: pip install pytoniq")
    ...
    print(f"   - /api/create-payload (NEW)")
